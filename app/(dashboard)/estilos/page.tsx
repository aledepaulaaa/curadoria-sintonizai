...
import { 
  listarEstilos, salvarEstilo, removerEstilo
...
export default function EstilosPage() {
  ...
  const handleAddItem = (grupoId: string) => {
    let nome = prompt('Nome do novo estilo:');
    if (!nome) return;
    nome = nome.trim();
    if (!nome) return;
    ...
    setGrupos(prev => prev.map(g => {
      if (g.id === grupoId) {
        if (g.itens?.some((i: string) => i.trim().toLowerCase() === nome?.toLowerCase())) {
          alert('Este estilo já existe neste grupo!');
          return g;
        }
  ...
  const handleRenameGroup = (grupoId: string) => {
    ...
    const novoNome = prompt('Novo nome para o grupo de estilos:', grupo.label);
    if (!novoNome) return;
    setGrupos(prev => prev.map(g => 
  ...
  const handleCreateGroup = () => {
    const nome = prompt('Nome do novo grupo de estilos (ex: Música, Teatro):');
    if (!nome) return;
  ...
  const handleRemoveGroup = async (id: string) => {
    if (!confirm('Tem certeza que deseja excluir este grupo de estilos? Isso afetará o app.')) return;
    try {
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 pb-20">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500 font-medium">Configure os grupos de estilos (ritmos, gêneros) e seus sub-filtros que aparecem no App Mobile.</p>
        </div>
      ...
      <div className="bg-purple-600/5 border border-purple-600/20 p-6 rounded-[2.5rem] flex gap-4 items-start">
         ...
         <p className="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">
            <strong>Organização Agrupada:</strong> Crie grupos temáticos de estilos (ex: "Música", "Teatro") para que o app possa carregar os filtros de forma hierárquica.
            O estado inicial é vazio para controle total da curadoria.
         </p>
      </div>
      ...
      {grupos.length === 0 ? (
        <div className="py-20 text-center bg-zinc-50 dark:bg-zinc-900/50 border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-[3rem]">
          <div className="max-w-xs mx-auto space-y-4">
            ...
            <h3 className="text-xl font-bold text-zinc-900 dark:text-white">Nenhum estilo definido</h3>
            <p className="text-sm text-zinc-500">A curadoria deve definir os estilos que serão exibidos para os usuários.</p>
            <button 
              onClick={handleCreateGroup}
